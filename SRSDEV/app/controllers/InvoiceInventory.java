...
public class InvoiceInventory extends Controller {
	...
	// private static String basePath = System.getenv("INVOICE_HOME"); 
	// private static String basePath = "/home/roots/web/pdfs/";
	private static String basePath = "/opt/pdfs/";
	...
	public static Result getInvoicesForPayment() {
		...
		if (storeId.equals("")) { // if he selects only company,present all invoices of that company
			...
			if (invoiceCategory.equals("Fuel")) { // gives Fuel Invoices of that Store
				paymentTerms = "Fuel";
		...
		// assign one Invoice id value to refInvoiceId ,if invoice list is not  empty
		if (invoiceList.size() >= 1) {
			refInvoiceId = invoiceList.get(0).id;
		}
		Logger.info("@C InvoiceInventory -->> getInvoicesForPayment(invoiceList, "+ paymentTerms + ") <<--");
	...
	public static Result dateSelectPageForHeadOffice() {
		...
		return ok(dateSelectForInvoiceViewForHeadOffice.render(invoiceList,
				null, Application.getDate(d), Application.getDate(d), null,
				null));
	}
	...
	public static Result getInvoicesBySupplierName() {
		...
		Logger.info("@C InvoiceInventory------------ -->> getInvoicesBySupplierName(supplier typed in search box is :"
				+ supplierName);
		invoiceList = Invoice.getInvoiceListBySupplierName(supplierName, sid1,
				invoiceStatus);
	...
	public static Result displayUploadedForHeadOffice() {
		...
		if (supplierName.equals("")) {
			System.out.println("====================== :"
					+ "u did not search by supplier");
			if (startDate.equals(endDate)) {

				invoiceList = Invoice.page(sid, Application.getDate(startDate),
						Application.getDate(startDate), invoiceStatus);
			...
			invoiceList = Invoice.getInvoiceListBySupplierName(supplierName,
					sid, invoiceStatus);
		 ...
		 * (supplierName.equals("") || invoiceStatus.equals(""))) { Logger.info(
		 * "@C InvoiceInventory -->> @@@@@@@@() -->>supplier_name,invoice_status -->> "
		 * +supplierName+","+invoiceStatus);
		 * 
		 * // if start Date and end date both are equal if
		 ...
		 * 
		 * invoiceList = Invoice.page(sid, Application.getDate(startDate),
		 * Application.getDate(endDate)); } } else{ if((!supplierName.equals("")
		 * || !(invoiceStatus.equals("")))){ Date date = new Date();
		 * System.out.println("DATE IS :"+date);
		 * startDate=Application.getDateString(date);
		 * endDate=Application.getDateString(date);
		 * Logger.info("@C InvoiceInventory -->>inside else-->> "
		 * +supplierName+","+invoiceStatus); invoiceList =
		 * Invoice.getInvoiceListBySupplierName( supplierName, sid,
		 * invoiceStatus); }
		 * 
		 * }
		 */
	...
	public static Result uploadInvoices(Long sid) {
		...
		String month_string = Application.getMonthForInt(month);
		// path to Upload Files
		// File ftemp= new File(basePath
		// +"//public//InvoiceInventory//"+store.name+"//SHOP//"+invoiceType+"//");
		// for play
		File ftemp = new File(basePath + "//InvoiceInventory//"+ year
				+ "//" + month_string + "//" + store.name + "//SHOP//"
				+ invoiceType + "//"); // for tomcat
		ftemp.mkdirs();
		...
		for (FilePart p : fileList) {
			...
			try {
				...
				String filePathString = "InvoiceInventory/" + year + "/"
						+ month_string + "/" + store.name + "/SHOP/"
						+ invoiceType + "/" + modifiedFileName;
	...
	public static Result uploadInvoicesByHeadOffice() throws Exception {
		...
		date = Application.getDate(date);
		Calendar cal = Calendar.getInstance();
		cal.setTime(Application.getDate(date));
		int month = cal.get(Calendar.MONTH);
		int year = cal.get(Calendar.YEAR);
		Logger.info("tODAY dATE IS : " + date);
		Logger.info("MONTH NUM FROM TODAY DATE IS: " + month);
		Logger.info("MONTH NUM FROM TODAY DATE IS: "
				+ Application.getMonthForInt(month));
		Logger.info("YEAR FROM TODAY DATE IS : " + year);
		play.mvc.Http.MultipartFormData body = request().body()
				.asMultipartFormData();

		// get InvoiceType from select Box
		String invoiceType = body.asFormUrlEncoded().get("invoiceType")[0];
		Logger.info("@C InvoiceInventory -->> uploadInvoicesByHeadOffice() -->>------------ "+invoiceType);
		Store store = Store.find.byId(Long.parseLong(body.asFormUrlEncoded()
				.get("storeId")[0]));
		Long sid = store.id;

		String monthName = Application.getMonthForInt(month);
		// path to Upload Files
		// File ftemp= new File(basePath
		// +"//public//InvoiceInventory//"+store.name+"//SHOP//"+invoiceType+"//");
		// for play
		...
		try {
			for (FilePart p : fileList) {
				...
				f1.setWritable(true);
				f1.setReadable(true);
				Logger.info("Inside try before file copy=================>");
				Logger.info("Inside try before file copy file.toPath()=================>"
						+ file.toPath());
				Logger.info("Inside try before file copy f1.toPath()=================>"
						+ f1.toPath());
				Files.copy(file.toPath(), f1.toPath(), REPLACE_EXISTING);
				Logger.info("Inside try after file copy=================>");
				String filePathString = "InvoiceInventory/" + year + "/"
						+ monthName + "/" + store.name + "/SHOP/"
						+ invoiceType + "/" + modifiedFileName;
				Logger.info("Inside try filePathString before file copy=================>"
						+ filePathString);
				Logger.info("Before checkFileExists()=================>");
				// update for File Replace & Create new Record For File Copy
				Invoice.checkFileExists(store.id, invoiceType,
						modifiedFileName, "UPLOADED", filePathString);
				Logger.info("After checkFileExists()=================>");
	...
	public static Result getInvoices() {
		...
		List<Invoice> invoiceList = Invoice.page(storeId, invoiceType,"UPLOADED");
	...
	public static Result move(Long id) {
		...
		if (paymentTerms == null) { // for fuel Invoices we disabled
									// paymentTerms.hence null will come if we
									// read it.Heance instead of Null i am
									// placing "Fuel" in DB record
		...
		if (invoiceCategory.equals("Fuel")) { // if it is Fuel category Place in
			...
			destPath = new File(basePath + "//InvoiceInventory//" + year + "//"
					+ monthString + "//" + store.name + "//HEAD OFFICE//"
					+ monthString + "//" + invoiceCategory + "//"); // for
																	// tomcat
			filePathString.append("InvoiceInventory/" + year + "/"
					+ monthString + "/" + store.name + "/HEAD OFFICE/"
					+ monthString + "/" + invoiceCategory + "/");
			...
			if (invoice.invoiceType.equals("CashPaid")) { // if invoice type is
				...
				destPath = new File(basePath + "//InvoiceInventory//" + year
						+ "//" + monthString + "//" + store.name
						+ "//HEAD OFFICE//" + monthString + "//" + paymentTerms
						+ "//" + supplier.name.trim() + "//PAID//"); // for
																		// tomcat
				filePathString.append("InvoiceInventory/" + year + "/"
						+ monthString + "/" + store.name + "/HEAD OFFICE/"
						+ monthString + "/" + paymentTerms + "/"
	...
	public static Result headOfficeMove(Long id) {
		...
		int month = cal.get(Calendar.MONTH);
		String monthString = Application.getMonthForInt(month);
		File destPath = new File(basePath + "//InvoiceInventory//" + year
				+ "//" + monthString + "//" + store.name + "//" + company.name
				+ "//" + monthString + "//" + invoiceCategory + "//"); // for
																		// tomcat
		destPath.mkdirs();
		File destPathFile = new File(destPath.getAbsolutePath() + "//"
		...
		filePathString.append("InvoiceInventory/" + year + "/" + monthString
				+ "/" + store.name + "/" + company.name + "/" + monthString
				+ "/" + invoiceCategory + "/");
		filePathString.append(invoice.fileName);
	...
	public static Result paymentMove(Long id, String paymentTerms,
		...
		Logger.info("@C InvoiceInventory --==================>> paymentMove(Invoice id :"
				+ id + "," + selectedCategory + ") -->> ");
		Invoice invoice = Invoice.find.byId(id);
		StringBuffer filePathString = new StringBuffer(); // to hold destination
															// file path
	...
	public static Result delete(Long id) {
		...
		Invoice invoice = Invoice.find.byId(id);
		Store store = Store.find.byId(invoice.storeId);
		Date invoiceDate = invoice.uploadedDate;
		Logger.info("@C InvoiceInventory -->> delete(Invoice id  year:"
				+ invoiceDate + ") <<--");
		// get Month from the Invoice Date
		Calendar cal = Calendar.getInstance();
		cal.setTime(Application.getDate(invoiceDate));
		...
		int month = cal.get(Calendar.MONTH);
		String monthString = Application.getMonthForInt(month);
		Logger.info("@C InvoiceInventory -->> delete(invoiceDate:"
				+ invoiceDate + ") <<--");
		Logger.info("@C InvoiceInventory -->> delete(Invoice id  year:" + year
				+ ") <<--");
		Logger.info("@C InvoiceInventory -->> delete(monthString:"
				+ monthString + ") <<--");
		// File file = new File(basePath
		// +"//public//InvoiceInventory//"+store.name+"//SHOP//"+invoice.invoiceType+"//");
		// // for play
		File file = new File(basePath + "//InvoiceInventory//" + year + "//"
				+ monthString + "//" + store.name + "//SHOP//"
				+ invoice.invoiceType + "//"); // for tomcat
