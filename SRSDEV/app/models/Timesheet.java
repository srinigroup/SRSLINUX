...
public class Timesheet extends Model {
	...
	public static List<Employee> getEmployeesInfoByTimesheet(Long sid,
			Date weekStartDate) {
		Logger.info("@M getEmployeesInfoByTimesheet=========>"+sid +","+weekStartDate);		
		...
		Logger.info("@M getEmployeesInfoByTimesheet=========>"+sid +","+employees.size());	
		return employees;
	}
	...
	public static List<Timesheet> getHeadOfficeEmployeeTimeSheetForSubmittedByWeekDate(
		...
		List<Timesheet> timeSheetList = new ArrayList<Timesheet>();
		timeSheetList = find.where().eq("firmid", storeId)
				.eq("status", "SUBMITTED").ge("date", stDate)
				.le("end_date", enDate).findList();
